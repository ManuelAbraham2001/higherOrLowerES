...
const CardsContainer = () => {
    ...
    // array con la id de los canales de youtube
    ...
    // obtengo aleatoreamente 2 ids para realizar el fetch

    let yt1 = youtubers[Math.floor(Math.random() * youtubers.length)];
    let yt2 = youtubers[Math.floor(Math.random() * youtubers.length)];
    
    // defino las variables del objeto youtuber

    let nombre = ''
    let suscriptores = ''
    let imagen = ''

    // defino el objeto youtuber

    let newYtber = {}

    // obtengo mediante fetch la informacion del youtuber mediante la id obtenida previamente
    ...
    let userInfo = async (setYoutuber, ytber) =>{
        try {
            ...
            // guardo los valores en las variables creadas previamente
        ...
        // agrego las variables a mi objeto
        ...
        // lo setteo en el youtuber 1 o 2 segun el parametro que venga en la funcion
    ...
    useEffect(() => {
        ...
        // si change esta en true se agrega una animacion de movimiento a la card, cuando termina se settea en false para que se le quite la clase
        setChange(false)

        // llamo a la funcion para obtener datos, pasandole los parametros de ambos youtubers
    ...
    let correct = () => {
        ...
        // setteo change en true para agregar la clase de la animacion
        ...
        // obtengo un id nuevo para la segunda card
        
        yt2 = youtubers[Math.floor(Math.random() * youtubers.length)];
        setTimeout(function(){
            // actualizo el contador de respuestas correctas
            setCorrectCount(correctCount + 1)
            // le paso la informacion del youtuber 2 a la card de la izquierda (youtuber 1)
            setYoutuber1(youtuber2)
            setCardActive(false)
            // con la id obtenida previamente llamo a la funcion que recopila los datos para settear un nuevo youtuber 2 (card de la derecha)
            userInfo(setYoutuber2, yt2)
            // setteo false para sacar la animacion
            setChange(false)
        }, 5000);
    }
    ...
    let incorrect = () => {
        setChange(true)
        setTimeout(function(){
            // reinicio el contador de respuestas correctas
            setCorrectCount(0)
            // setteo el estado del juego a false para que se renderee el boton de volver a jugar
            setGameStatus(false)
    ...
    let more = () => {
        // comparo los suscriptores de ambos youtubers
        if (youtuber2.suscriptores >= youtuber1.suscriptores) {
            // en caso de que el youtuber 2 sea la respuesta correcta se ejecuta la funcion correct()
            correct()
        } else {
            // sino se ejecuta incorrect()
            incorrect()
            return
        }
    }

    // boton de 'menos' idem al boton de pero al reves 'mas'
    ...
    // funcion que se ejecuta al dar click a jugar de nuevo, reinicia todas las variables y ejecuta la funcion de obtener la info del user
  ...
  return (
    <>
    {gameStatus ?    <div className="card_container">
