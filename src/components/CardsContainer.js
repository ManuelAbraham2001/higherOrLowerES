...
const CardsContainer = () => {
    ...
    let userInfo2 = async () =>{
        try {
            let res = await fetch(`https://www.googleapis.com/youtube/v3/channels?part=snippet&id=${yt2}&key=${process.env.REACT_APP_API_KEY}`)
            let data = await res.json()
    
            let respuesta = await fetch(`https://www.googleapis.com/youtube/v3/channels?part=statistics&id=${yt2}&key=${process.env.REACT_APP_API_KEY}`)
    ...
    useEffect(() => {
        ...
        let userInfo1 = async () =>{
            try {
                let res = await fetch(`https://www.googleapis.com/youtube/v3/channels?part=snippet&id=${yt1}&key=${process.env.REACT_APP_API_KEY}`)
                let data = await res.json()
        
                let respuesta = await fetch(`https://www.googleapis.com/youtube/v3/channels?part=statistics&id=${yt1}&key=${process.env.REACT_APP_API_KEY}`)
                let datos = await respuesta.json()
  ...
  return (
    ...
    <Card name={youtuber1.nombre} suscribers={youtuber1.suscriptores} pp={youtuber1.imagen}></Card>
    <h1>VS</h1>
    <h2>{correctCount}</h2>
    <Card2 name={youtuber2.nombre} suscribers={youtuber2.suscriptores} more={more} less={less} pp={youtuber2.imagen} conditionRender={change}></Card2>
    </div>
  )
}
